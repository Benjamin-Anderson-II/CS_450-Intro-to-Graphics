...
const char *WINDOWTITLE = "OpenGL / CS_450 Project 3 -- Benjamin Anderson II";
...
GLuint	CatDL;				// object display list
GLuint	TreeDL;				// object display list
GLuint	SkullDL;				// object display list
...
Animate( )
{
    ...
    LightX = 9*cos(25*Time);
    LightZ = 9*sin(25*Time);
...
Display( )
{
    ...
    // so that we can have proper lighting
    ...
    glPushMatrix();
        ...
        glRotatef(-90, 1, 0, 0);
        glRotatef(45, 0, 0, 1);
        SetMaterial(0.1, 0.1, 0.9, 10);
        glCallList( SkullDL );
    glPopMatrix();

    // Light Source
    if(LightType == POINT)
        SetPointLight(GL_LIGHT0, LightX,12,LightZ,         LightR,LightG,LightB);
    else if(LightType == SPOT)
        SetSpotLight (GL_LIGHT0, LightX,12,LightZ, 0,-1,0, LightR,LightG,LightB);
    glPushMatrix();
        glDisable(GL_LIGHTING);
        glTranslatef(LightX,12,LightZ);
        glScalef(0.25, 0.25, 0.25);
...
InitLists( )
{
    ...
    SkullDL = glGenLists(1);
    glNewList(SkullDL, GL_COMPILE);
        LoadObjFile((char *)"Skull.obj");
    glEndList();
