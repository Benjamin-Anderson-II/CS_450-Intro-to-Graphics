...
GLuint	MoonDL;				// object display list
...
GLuint	PlutoDL;				// object display list
...
Animate( )
{
    ...
    LightX = 12*cos(25*Time);
    LightZ = 12*sin(25*Time);
    LightY = 12*sin(12.5*Time);
...
InitGraphics( )
{
    ...
    for(int i = 0; i < 8; i++){
        texture = BmpToTexture(files[i], &width, &height);
        if(!texture)
...
InitLists( )
{
    ...
    glNewList(VenusDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[VENUS]);
        glPushMatrix();
    ...
    glNewList(EarthDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[EARTH]);
        glPushMatrix();
            glScalef(1, 1, 1);
            glCallList(SphereDL);
        glPopMatrix();
    ...
    glNewList(MoonDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[MOON]);
        glPushMatrix();
    ...
    glNewList(JupiterDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[JUPITER]);
        glPushMatrix();
    ...
    glNewList(SaturnDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[SATURN]);
        glPushMatrix();
    ...
    glNewList(UranusDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[URANUS]);
        glPushMatrix();
    ...
    glNewList(NeptuneDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[NEPTUNE]);
        glPushMatrix();
    ...
    glNewList(PlutoDL, GL_COMPILE);
        glBindTexture(GL_TEXTURE_2D, PlanetTextures[PLUTO]);
        glPushMatrix();
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
        ...
        case 't':
        case 'T':
            doTexture = (TextureVals)((doTexture + 1) % 3);
            break;
